...
include RoleAccess
  attr_accessible :local_addr, :perm_addr, :age, :designation, :dob, :doj, :emp_id, :first_name, :last_name, :manager_emp_id, :manager_name, :mob_no, :off_email_id, :pan_no, :pers_email_id, :phone_no, :gender, :seperation_date, :encashed_days, :roles
attr_accessible :leaves_attributes, :roles, :leave_attributes
...
has_many :articles, :class_name => 'Article', :foreign_key => 'employee_id'
...
def self.search(search)
   if search
    if search.is_a?(String)
       where('emp_id LIKE ? or last_name LIKE ? or first_name LIKE ? or designation LIKE ?', "%#{search}%" , "%#{search}%" , "%#{search}%", "%#{search}%")
    end
@salaryst=Employee.where("id=?", @salary.empid).pluck(:empid)[0]
   else
       scoped
   end
end
...
manager=self
@reportees=Array.new
   find_reportees(manager.emp_id)
end

def find_reportees(emp_id)
      rep = Employee.find_all_by_manager_emp_id(emp_id)
      for i in 0..rep.count-1
      	@reportees << rep[i]
      	find_reportees(rep[i].emp_id)
      end
      @reportees
end
...
def is_manager?
    ["Manager","SSE","Senior Manager"].include? self.designation
end
